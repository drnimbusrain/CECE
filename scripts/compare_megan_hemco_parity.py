...
def _map_ax(ax, data, lons, lats, title, units, vmin=None, vmax=None,
            cmap="YlOrRd", diverging=False):
    ...
    cf = ax.contourf(lon2d, lat2d, data, levels=20,
    ...
    ax.set_xlim(-180, 180)
    ax.set_ylim(-90, 90)
    plt.colorbar(cf, ax=ax, orientation="horizontal", pad=0.12,
                 label=units, fraction=0.046)
...
def plot_global_maps(hemco, native, megan3, lons, lats, outdir):
    ...
    fig.suptitle("MEGAN isoprene emission comparison — 2021-06-20 daily mean",
                 fontsize=11)
    fig.tight_layout()
...
def plot_zonal(hemco, native, megan3, lats, outdir):
    ...
    ax.plot(np.nanmean(native, axis=1), lats, label="CECE native",     color="#1f77b4", lw=1.5, ls="--")
    ax.plot(np.nanmean(megan3, axis=1), lats, label="CECE MEGAN3 ISOP", color="#d62728", lw=1.5, ls=":")
...
def summary_stats(hemco, test, label):
    ...
    return {"label": label, "mean_ref": mean_h, "mean_test": mean_t,
...
def main():
    ...
    parser.add_argument("--hemco",  required=True,
                        help="NetCDF from cece_config_hemco_megan_parity.yaml")
    parser.add_argument("--native", required=True,
                        help="NetCDF from cece_config_megan_hemco_comparison.yaml")
    parser.add_argument("--megan3", required=True,
    ...
    plot_bias(native - hemco, lons, lats,
              "CECE native − HEMCO 3.12.1",
              "isop_bias_native_vs_hemco.png", args.outdir)
    plot_bias(megan3 - hemco, lons, lats,
              "CECE MEGAN3 − HEMCO 3.12.1",
              "isop_bias_megan3_vs_hemco.png", args.outdir)
    ...
    with open(summary_path, "w") as fh:
        ...
        fh.write("=" * 50 + "\n")
        fh.write(f"Reference: HEMCO 3.12.1 stateless (megan_method=hemco_3_12_1)\n")
        fh.write(f"Grid:      global 4°×5° (72×46), 2021-06-20 daily mean\n")
        fh.write(f"Units:     mg C m⁻² hr⁻¹\n\n")
    ...
    for s in (stats_native, stats_megan3):
        print(f"  {s['label']:30s}  bias={s['bias']:+.4f}  "
