...
def get_gamma_age(cmlai, pmlai, dbtwn, tt, an, ag, am, ao):
    ...
    cmlai, pmlai, dbtwn, tt = np.broadcast_arrays(
        np.asarray(cmlai, dtype=float), np.asarray(pmlai, dtype=float),
        np.asarray(dbtwn, dtype=float), np.asarray(tt, dtype=float),
    )
    ...
    with np.errstate(divide="ignore", invalid="ignore"):
        fnew_grow = np.where(dbtwn > ti, (ti / dbtwn) * (1.0 - pmlai / cmlai), 1.0 - (pmlai / cmlai))
        fmat_grow = np.where(dbtwn > tm, (pmlai / cmlai) + ((dbtwn - tm) / dbtwn) * (1.0 - pmlai / cmlai), pmlai / cmlai)
...
def get_gamma_sm(gwetroot, is_ald2_or_eoh=False):
    ...
    return np.ones_like(gwetroot) if not is_ald2_or_eoh else np.maximum(20.0 * np.clip(gwetroot, 0.0, 1.0) - 17.0, 1.0)
...
def get_gamma_par_pceea(q_dir, q_diff, par_avg, suncos, doy, wm2_to_umol, ptoa_c1, ptoa_c2, gp1, gp2, gp3, gp4):
...
ISOP_ANEW, ISOP_AGRO, ISOP_AMAT, ISOP_AOLD = 0.05, 0.6, 1.0, 0.9  # kDefault{Anew,Agro,Amat,Aold}[0]
...
def megan3_isop_activity_factor_cece(T, L, L_prev, pdr, pdf, sc, gwetroot):
    ...
    g_age_c = get_gamma_age(L, L_prev, CECE_DBTWN, T, ISOP_ANEW, ISOP_AGRO, ISOP_AMAT, ISOP_AOLD)
    g_sm = get_gamma_sm(gwetroot, is_ald2_or_eoh=False)
    g_t_li = get_gamma_t_li(T, ISOP_BETA, 303.0)
    g_t_ld = get_gamma_t_ld(T, CECE_T_AVG_15, ISOP_CT1, ISOP_CLEO, GAS_CONSTANT, CT2_CONST, T_OPT_C1, T_OPT_C2, E_OPT_COEFF)
    g_par = get_gamma_par_pceea(pdr, pdf, CECE_PAR_AVG, sc, CECE_DOY, WM2_TO_UMOL, PTOA_C1, PTOA_C2, GP_C1, GP_C2, GP_C3, GP_C4)
...
def megan_isop_activity_factor_hemco(T, L, L_prev, pdr, pdf, sc, co2_ppm=390.0):
    ...
    g_age = get_gamma_age(L, L_prev, HEMCO_DBTWN, T, ISOP_ANEW, ISOP_AGRO, ISOP_AMAT, ISOP_AOLD)
    g_t_li = get_gamma_t_li(T, ISOP_BETA, 303.0)
    g_t_ld = get_gamma_t_ld(T, HEMCO_T_HISTORY, ISOP_CT1, ISOP_CLEO, GAS_CONSTANT, CT2_CONST, T_OPT_C1, T_OPT_C2, E_OPT_COEFF)
    g_par = get_gamma_par_pceea(pdr, pdf, HEMCO_PARDR_HISTORY + HEMCO_PARDF_HISTORY, sc, HEMCO_DOY,
                                 WM2_TO_UMOL, PTOA_C1, PTOA_C2, GP_C1, GP_C2, GP_C3, GP_C4)
...
def bdsnp_ndep_factor(ndep, fert_ef=BDSNP_FERT_EF, wet=BDSNP_WET_DEP_SCALING, dry=BDSNP_DRY_DEP_SCALING):
...
def bdsnp_soil_no_emission(soil_temp_k, soil_moisture, lai, ndep=0.0, base_ef=1.0):
    ...
    emiss = base_ef * BDSNP_UNITCONV * t_response * sm_factor * fert_factor * canopy_red * pulse
...
def build_synthetic_fields():
    ...
    lai = base_lai * land
    lai_prev = np.clip(lai * 0.92, 0.0, None)  # 8-day-earlier MODIS composite, slightly less green-up

    solar = np.clip(np.cos(np.radians(lat2d - 23.0)) * 0.85, 0.0, 1.0)  # local-noon proxy, July NH-summer offset
    ...
    return {
        "lon2d": lon2d, "lat2d": lat2d, "land": land,
        "leaf_area_index": lai, "leaf_area_index_prev": lai_prev,
        "par_direct": par_direct, "par_diffuse": par_diffuse, "solar_cosine": solar_cosine,
...
def run_scalar_case_validation(outdir: Path):
    csv_path = REPO_ROOT / "tests" / "data" / "hemco_megan" / "hemco_3_12_1_megan_reference.csv"
    rows = []
    ...
    out_rows = []
    for row in rows:
        T = float(row["T_K"]); L = float(row["lai_m2m2"]); Lp = float(row["lai_prev_m2m2"])
        pdr = float(row["pardr_Wm2"]); pdf = float(row["pardf_Wm2"]); sc = float(row["suncos"])
        gw = float(row["gwetroot"]); co2 = float(row["co2_ppm"])
        ...
        rel_err_hemco_repro = abs(hemco_val - expected) / expected if expected != 0.0 else (0.0 if hemco_val == 0.0 else float("inf"))
        rel_diff_cece_vs_hemco = abs(cece_val - hemco_val) / expected if expected != 0.0 else (0.0 if cece_val == 0.0 else float("inf"))
        out_rows.append({
            "case_id": row["case_id"],
...
def make_plots(fields, isop_emission, soil_no_emission, isop_activity_cece, isop_activity_hemco, outdir: Path):
    import matplotlib
    matplotlib.use("Agg")
    ...
    isop_mgC = isop_emission * 1.0e6 * 3600.0 * (5.0 * 12.011 / 68.12)  # kg m-2 s-1 -> mg C m-2 hr-1
    vmax = float(np.percentile(isop_mgC, 99.0)) or 1.0
    im = ax.pcolormesh(lon2d, lat2d, isop_mgC, shading="auto", cmap="YlOrRd", vmin=0.0, vmax=vmax)
    ax.set(title="CECE native MEGAN3 ISOP emission (4x5 test grid)",
    ...
    fig, ax = plt.subplots(figsize=(10, 5), constrained_layout=True)
    soil_no_ngNm2s = soil_no_emission * 1.0e12 / BDSNP_MW_NO * 14.0  # kg NO -> ng N m-2 s-1
    vmax_no = float(np.percentile(soil_no_ngNm2s, 99.0)) or 1.0
    im = ax.pcolormesh(lon2d, lat2d, soil_no_ngNm2s, shading="auto", cmap="YlGnBu", vmin=0.0, vmax=vmax_no)
    ax.set(title="CECE native BDSNP soil NO emission (4x5 test grid)",
           xlabel="Longitude [deg]", ylabel="Latitude [deg]")
    ...
    with np.errstate(divide="ignore", invalid="ignore"):
        pct_diff = 100.0 * (isop_activity_cece - isop_activity_hemco) / np.where(isop_activity_hemco != 0.0, isop_activity_hemco, np.nan)
    fig, ax = plt.subplots(figsize=(10, 5), constrained_layout=True)
    ...
    im = ax.pcolormesh(lon2d, lat2d, np.clip(pct_diff, -limit, limit), shading="auto", cmap="RdBu_r", vmin=-limit, vmax=limit)
    ax.set(title="CECE native MEGAN3 vs HEMCO 3.12.1 source oracle: % diff in ISOP activity factor\n"
    ...
    ax.plot(np.nanmean(isop_activity_cece, axis=1), LAT, label="CECE native MEGAN3 (current hard-coded history)")
    ax.plot(np.nanmean(isop_activity_hemco, axis=1), LAT, label="HEMCO 3.12.1 source oracle (PR #90)")
    ax.set(title="Zonal-mean ISOP activity factor", xlabel="Activity factor per unit AEF", ylabel="Latitude [deg]")
    ax.grid(alpha=0.3)
    ax.legend()
...
def main():
    ...
    parser.add_argument("--config", type=Path, default=REPO_ROOT / "tests" / "cece_config_earthaccess_4x5_test.yaml")
    parser.add_argument("--outdir", type=Path, default=REPO_ROOT / "results" / "bdsnp_megan3_4x5_test")
    ...
    assert "megan3" in schemes and "bdsnp" in schemes, "config must request megan3 and bdsnp"

    fields = build_synthetic_fields()

    isop_activity_cece = megan3_isop_activity_factor_cece(
        fields["temperature"], fields["leaf_area_index"], fields["leaf_area_index_prev"],
        fields["par_direct"], fields["par_diffuse"], fields["solar_cosine"], fields["soil_moisture_root"],
    )
    isop_activity_cece = np.where(fields["leaf_area_index"] > 0.0, isop_activity_cece, 0.0)
    isop_emission = ISOP_AEF_DEFAULT * isop_activity_cece

    isop_activity_hemco = megan_isop_activity_factor_hemco(
        fields["temperature"], fields["leaf_area_index"], fields["leaf_area_index_prev"],
        fields["par_direct"], fields["par_diffuse"], fields["solar_cosine"], CECE_CO2_PPM,
    )
    isop_activity_hemco = np.where(fields["leaf_area_index"] > 0.0, isop_activity_hemco, 0.0)

    soil_no_emission = bdsnp_soil_no_emission(
        fields["soil_temperature"], fields["soil_moisture"], fields["leaf_area_index"],
        ndep=0.0, base_ef=1.0,
    )

    args.outdir.mkdir(parents=True, exist_ok=True)
    make_plots(fields, isop_emission, soil_no_emission, isop_activity_cece, isop_activity_hemco, args.outdir)
    scalar_rows, scalar_csv_path = run_scalar_case_validation(args.outdir)

    valid = fields["leaf_area_index"] > 0.0
    with np.errstate(divide="ignore", invalid="ignore"):
        pct_diff = 100.0 * (isop_activity_cece[valid] - isop_activity_hemco[valid]) / isop_activity_hemco[valid]
    pct_diff = pct_diff[np.isfinite(pct_diff)]

    hemco_repro_err = max(r["hemco_reproduction_rel_err"] for r in scalar_rows if math.isfinite(r["hemco_reproduction_rel_err"]))
    cece_vs_hemco_scalar = [r["cece_vs_hemco_rel_diff"] for r in scalar_rows if math.isfinite(r["cece_vs_hemco_rel_diff"])]
