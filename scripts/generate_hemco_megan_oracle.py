...
def gamma_par(pardr, pardf, suncos, doy=REF_DOY):
    ...
    ptoa  = PTOA_C1 + PTOA_C2 * math.cos(
        2.0 * math.pi * (doy - PTOA_DOY_OFFSET) / 365.0
    )
...
def gamma_age(cml, pml, T):
    ...
    if cml == pml:
        ...
        fmat = ((pml / cml) + ((DBTWN - tm) / DBTWN) * (1 - pml / cml)
                if DBTWN > tm else pml / cml)
        fgro = 1 - fnew - fmat
        fold = 0.0
    else:
        fnew, fgro = 0.0, 0.0
...
def main():
    ...
    for (cid, T, lai, lprev, pdr, pdf, sc, gw, co2) in CASES:
        ...
        print(f"{cid},{T},{lai},{lprev},{pdr},{pdf},{sc},{gw},{co2},"
              f"{v:.18e}")
