...
The staging helper downloads protected granules into each staged step's `granules/` cache before opening them with `xarray+h5netcdf`. The `cloud` extra explicitly installs `h5py`, which provides h5netcdf's HDF5 backend. Streams should use NetCDF4/HDF5-readable collections; HDF-EOS `.hdf` granules are detected during preflight and should be pre-converted to NetCDF before compute-node runs.
```bash
