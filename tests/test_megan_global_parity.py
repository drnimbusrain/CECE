...
LDF      = 0.9996
BETA     = 0.13;  T_STD = 303.0
R        = 8.3144598e-3
CT1      = 95.0;  CEO = 2.0;  CT2 = 200.0
T_OPT_C1 = 313.0; T_OPT_C2 = 0.6; E_OPT_C = 0.08
WM2      = 4.766
# HEMCO 3.12.1 PTOA
H_PTOA_C1 = 2650.0; H_PTOA_C2 = 130.0; H_DOY_OFF = 18.0
H_PAR_AVG = 400.0   # direct µmol/m²/s
# CECE native PTOA
N_PTOA_C1 = 3000.0; N_PTOA_C2 = 99.0; N_DOY_OFF = 10.0
N_PAR_AVG = 400.0   # W/m² (converted internally)
# Shared
GCO2_C1 = 8.9406; GCO2_C2 = 0.0024
LAI_C1  = 0.49;   LAI_C2  = 0.2
...
def global_fields():
    ...
    def base_lai(lat):
        a = np.abs(lat)
        return np.where(a < 15, 5.0,
               np.where(a < 35, 3.5,
               np.where(a < 55, 2.5,
               np.where(a < 70, 1.5, 0.4))))

    desert = np.where(
        ((LON > -20) & (LON < 55) & (LAT > 18) & (LAT < 32)) |
        ((LON > 45)  & (LON < 65) & (LAT > 20) & (LAT < 35)) |
        ((LON > -115)& (LON < -75)& (LAT > 20) & (LAT < 32)), 0.3, 1.0)
    # Apply a land-fraction threshold so pure-ocean cells have LAI=0 exactly
    lai = np.where(land < 0.05, 0.0,
                   np.clip(base_lai(LAT) * land * desert, 0, 8))

    def base_aef(lat):
        a = np.abs(lat)
        return np.where(a < 15, 3.0e-9,
               np.where(a < 25, 2.0e-9,
               np.where(a < 45, 1.5e-9,
               np.where(a < 60, 0.8e-9, 0.3e-9))))
    aef = np.where(lai > 0, base_aef(LAT) * land, 0.0)

    T = 302.0 - 0.10 * np.abs(LAT) - np.where(np.abs(LAT) > 20,
                                                0.05 * (np.abs(LAT) - 20) ** 2, 0)
    ...
    return dict(land=land, lai=lai, aef=aef, T=T, sc=sc, pdr=pdr, pdf=pdf,
                hemco_emis=hemco_emis, native_emis=native_emis)
...
def test_tropical_land_cells_positive(global_fields):
    ...
    mask = (np.abs(LAT) < 20) & (global_fields["land"] > 0.5) & (global_fields["lai"] > 0.5)
...
def test_zonal_mean_peak_in_tropics(global_fields):
    ...
    for label, e in [("HEMCO", global_fields["hemco_emis"]),
...
def test_monotonic_lai_scaling(global_fields):
    ...
    e_full = emission(global_fields["T"], lai_full,
                      global_fields["pdr"], global_fields["pdf"],
                      global_fields["sc"],  global_fields["aef"], hemco=True)
    e_half = emission(global_fields["T"], lai_half,
                      global_fields["pdr"], global_fields["pdf"],
                      global_fields["sc"],  global_fields["aef"] * 0.5, hemco=True)
    mask = lai_full > 1.0
    failures = int((e_half[mask] >= e_full[mask]).sum())
    total = int(mask.sum())
    assert failures == 0, \
        f"LAI monotonicity violated in {failures}/{total} cells"


def test_co2_inhibition_preserved(global_fields):
    """Doubling CO₂ (560 vs 390 ppm) reduces global emission by 20–50%."""
    f = global_fields
    e_390 = emission(f["T"], f["lai"], f["pdr"], f["pdf"], f["sc"], f["aef"],
                     co2=390.0, hemco=True)
    e_560 = emission(f["T"], f["lai"], f["pdr"], f["pdf"], f["sc"], f["aef"],
                     co2=560.0, hemco=True)
...
if __name__ == "__main__":
    ...
    def base_lai(lat):
        a = np.abs(lat)
        return np.where(a<15,5.0,np.where(a<35,3.5,np.where(a<55,2.5,np.where(a<70,1.5,0.4))))
    desert = np.where(
        ((LON>-20)&(LON<55)&(LAT>18)&(LAT<32))|
        ((LON>45)&(LON<65)&(LAT>20)&(LAT<35))|
        ((LON>-115)&(LON<-75)&(LAT>20)&(LAT<32)),0.3,1.0)
    lai = np.clip(base_lai(LAT)*land*desert,0,8)
    aef = np.where(lai>0, np.where(np.abs(LAT)<15,3e-9,np.where(np.abs(LAT)<25,2e-9,
                   np.where(np.abs(LAT)<45,1.5e-9,np.where(np.abs(LAT)<60,0.8e-9,0.3e-9))))*land,0)
    T = np.clip(302-0.10*np.abs(LAT)-np.where(np.abs(LAT)>20,0.05*(np.abs(LAT)-20)**2,0)
                +np.where((land>0.5)&(LAT>0),2,0)+np.where(LAT<-60,-20,0),255,312)
    sc = np.clip(np.cos(np.radians(LAT-23))*0.85,0,1)
    rng=np.random.RandomState(42)
    pdr=np.clip(400*sc+20*rng.randn(NY,NX)*sc,0,700)
    pdf=np.clip(120*sc+10*rng.randn(NY,NX)*sc,0,350)
