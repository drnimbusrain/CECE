...
inline double cell_lon(int i) { return kLonFirst + i * kLonStep; }
...
inline int idx(int i, int j) { return j * kNX + i; }  // row-major [lat,lon]

// ============================================================================
// Analytical continental land fraction
// ============================================================================

static double smooth_box(double lon, double lat,
                         double lon1, double lon2,
                         double lat1, double lat2,
                         double edge = 3.0) {
    auto s = [edge](double x, double a, double b) -> double {
        return (std::tanh((x - a) / edge) - std::tanh((x - b) / edge)) * 0.5;
    };
    return std::clamp(s(lon, lon1, lon2) * s(lat, lat1, lat2), 0.0, 1.0);
}

static double smooth_ell(double lon, double lat,
                          double lon0, double lat0,
                          double rlon, double rlat,
                          double edge = 4.0) {
    double r = std::sqrt(std::pow((lon - lon0) / rlon, 2) +
                         std::pow((lat - lat0) / rlat, 2));
...
struct GlobalFields {
    ...
    GlobalFields() : land_frac(kNX*kNY), aef(kNX*kNY), T(kNX*kNY),
                     lai(kNX*kNY), pardr(kNX*kNY), pardf(kNX*kNY),
                     suncos(kNX*kNY) {
        ...
        for (int j = 0; j < kNY; ++j) {
            ...
            const double sc = std::clamp(
                std::cos(std::numbers::pi / 180.0 * (lat - 23.0)) * 0.85, 0.0, 1.0);
            ...
            for (int i = 0; i < kNX; ++i) {
                ...
                const double L = std::clamp(
                    biome_base_lai(abs_lat) * lf * desert_scale, 0.0, 8.0);
                lai[k] = L;
...
double emit_hemco3121(double T, double L, double Lprev,
                      double pdr, double pdf, double sc,
                      double aef_val, double co2 = 390.0) {
    MeganInputs in;
    in.T_K = T; in.lai = L; in.lai_prev = Lprev;
    in.pardr_Wm2 = pdr; in.pardf_Wm2 = pdf; in.suncos = sc;
    in.co2_ppm = co2;
...
double emit_native(double T, double L, double Lprev,
                   double pdr, double pdf, double sc,
                   double aef_val, double co2 = 390.0) {
    if (L <= 0.0) return 0.0;
    ...
    const double gage = (L == Lprev)
        ? 0.0*ANEW + 0.1*AGRO + 0.8*AMAT + 0.1*AOLD
        : GammaAge(L, Lprev, T);  // reuse HEMCO header for age calc
    ...
    const double gtld  = std::max(e_opt * CT2 * std::exp(CT1 * x) /
                                  (CT2 - CT1 * (1.0 - std::exp(CT2 * x))), 0.0);

    double gpar = 0.0;
    if (sc > 0.0) {
        const double pac_i = (pdr + pdf) * WM2_TO_UMOL;
        const double pac_d = PAR_AVG_WM2 * WM2_TO_UMOL;  // 1906 µmol/m²/s
        const double ptoa  = PTOA_C1 + PTOA_C2 *
                             std::cos(2.0 * std::numbers::pi * (DOY - DOY_OFF) / 365.0);
    ...
    static void SetUpTestSuite() {
        ...
        for (int j = 0; j < kNY; ++j) {
            for (int i = 0; i < kNX; ++i) {
                ...
                emis_hemco_[k]  = emit_hemco3121(gf.T[k], L, L,
                                                  gf.pardr[k], gf.pardf[k],
                                                  gf.suncos[k], gf.aef[k]);
                emis_native_[k] = emit_native(   gf.T[k], L, L,
                                                  gf.pardr[k], gf.pardf[k],
                                                  gf.suncos[k], gf.aef[k]);
            }
        }
...
TEST_F(HemcoMeganGlobalTest, OceanCellsZeroEmission) {
    ...
    EXPECT_EQ(violated, 0)
        << "LAI=0 cells must produce zero emission; " << violated << " cell(s) violated";
}
...
TEST_F(HemcoMeganGlobalTest, TropicalLandCellsPositive) {
    ...
    EXPECT_EQ(positive, total)
        << positive << "/" << total << " tropical land cells are positive";
}
...
TEST_F(HemcoMeganGlobalTest, TropicalDominance) {
    ...
    EXPECT_GT(fraction, 0.35)
        << "Tropical fraction = " << fraction << " (expected > 0.35)";
}
...
TEST_F(HemcoMeganGlobalTest, SpatialCorrelationHigh) {
    ...
    EXPECT_GT(R, 0.995)
        << "Spatial correlation R = " << R << " (expected > 0.995)";
}
...
TEST_F(HemcoMeganGlobalTest, ZonalMeanPeakInTropics) {
    auto peak_lat_idx = [&](const std::vector<double>& e) -> int {
        ...
        for (int j = 0; j < kNY; ++j) {
            ...
            if (s > best_mean) { best_mean = s; best_j = j; }
        }
        return best_j;
    };
    ...
    EXPECT_LT(std::abs(cell_lat(j_hemco)),  30.0)
        << "HEMCO 3.12.1 zonal peak at lat=" << cell_lat(j_hemco);
    EXPECT_LT(std::abs(cell_lat(j_native)), 30.0)
        << "CECE native zonal peak at lat=" << cell_lat(j_native);
}
...
TEST_F(HemcoMeganGlobalTest, MonotonicLAIScaling) {
    ...
    for (int j = 0; j < kNY; ++j) {
        for (int i = 0; i < kNX; ++i) {
            ...
            using namespace cece::hemco_megan::v3_12_1;
            MeganInputs in;
            in.T_K = fields_->T[k]; in.lai = L; in.lai_prev = L;
            in.pardr_Wm2 = fields_->pardr[k]; in.pardf_Wm2 = fields_->pardf[k];
            in.suncos = fields_->suncos[k]; in.co2_ppm = 390.0;
            in.par_avg_umol = kParAvgUmol; in.T_avg_15_K = kTAvg15; in.doy = kReferenceDoy;
    ...
    EXPECT_GT(pass_frac, 0.99)
        << "LAI monotonicity violated in " << (cells_tested - cells_correct) << "/" << cells_tested << " cells";
}
...
TEST_F(HemcoMeganGlobalTest, CO2InhibitionPreserved) {
    ...
    for (int j = 0; j < kNY; ++j) {
        for (int i = 0; i < kNX; ++i) {
            ...
            MeganInputs in;
            in.T_K = fields_->T[k]; in.lai = fields_->lai[k];
    ...
    EXPECT_LT(inhibition, 0.80)
        << "CO2 inhibition ratio = " << inhibition << " (expected < 0.80)";
    EXPECT_GT(inhibition, 0.50)
        << "CO2 inhibition ratio = " << inhibition << " (expected > 0.50)";
}
