...
TEST_F(HemcoParityTest, MeganStatelessGlobal4x5Parity) {
    const int nx = 72;
    const int ny = 46;
    const int nz = 1;
    const int npft = 16;
    ...
    DualView3D dv_pft_cpp("pft_fractions", nx, ny, npft);
    DualView3D dv_pft_fort("pft_fractions", nx, ny, npft);
    auto h_pft_cpp = dv_pft_cpp.view_host();
    auto h_pft_fort = dv_pft_fort.view_host();
    for (int i = 0; i < nx; ++i) {
        for (int j = 0; j < ny; ++j) {
            for (int p = 0; p < npft; ++p) {
                double val = (p == (i + j) % npft) ? 1.0 : 0.0;
                h_pft_cpp(i, j, p) = val;
                h_pft_fort(i, j, p) = val;
            }
        }
    }
    dv_pft_cpp.modify<Kokkos::HostSpace>();
    dv_pft_cpp.sync<Kokkos::DefaultExecutionSpace>();
    dv_pft_fort.modify<Kokkos::HostSpace>();
    dv_pft_fort.sync<Kokkos::DefaultExecutionSpace>();

    imp_cpp.fields["pft_fractions"] = dv_pft_cpp;
    imp_fort.fields["pft_fractions"] = dv_pft_fort;
    ...
    for (int i = 0; i < nx; ++i) {
        for (int j = 0; j < ny; ++j) {
            ...
            EXPECT_NEAR(val_cpp, val_fort, tol) << "MEGAN PFT 4x5 parity mismatch at (" << i << "," << j << ")";
        }
    }
#else
